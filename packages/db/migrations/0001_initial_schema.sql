-- Focus Reader: Initial Schema
-- Creates ALL tables upfront to avoid mid-flight migrations.

-- ============================================================
-- Core tables (Phase 0 active)
-- ============================================================
...
CREATE INDEX idx_document_location ON document(location);
CREATE INDEX idx_document_type ON document(type);
CREATE INDEX idx_document_source_id ON document(source_id);
CREATE INDEX idx_document_is_read ON document(is_read);
CREATE INDEX idx_document_is_starred ON document(is_starred);
CREATE INDEX idx_document_saved_at ON document(saved_at);
CREATE INDEX idx_document_deleted_at ON document(deleted_at);

CREATE TABLE document_email_meta (
...
CREATE UNIQUE INDEX idx_email_meta_message_id ON document_email_meta(message_id) WHERE message_id IS NOT NULL;
CREATE UNIQUE INDEX idx_email_meta_fingerprint ON document_email_meta(fingerprint) WHERE fingerprint IS NOT NULL;
...
CREATE INDEX idx_attachment_document_id ON attachment(document_id);
...
CREATE INDEX idx_ingestion_log_channel_type ON ingestion_log(channel_type);
CREATE INDEX idx_ingestion_log_status ON ingestion_log(status);
CREATE INDEX idx_ingestion_log_received_at ON ingestion_log(received_at);

-- Join tables (Phase 0 active)
...
CREATE TABLE document_pdf_meta (
  document_id TEXT PRIMARY KEY REFERENCES document(id) ON DELETE CASCADE,
...
CREATE TABLE ingestion_report_daily (
